...
def load_font(size=16):
    ...
    if FONT is None or FONT.size != size:
        try:
            if not os.path.exists("fonts/arial.ttf"):
                logging.error("Файл шрифта fonts/arial.ttf не найден")
                raise FileNotFoundError("Шрифт arial.ttf отсутствует")
            FONT = ImageFont.truetype("fonts/arial.ttf", size)
            logging.info(f"Шрифт arial.ttf успешно загружен, размер {size}")
        except Exception as e:
            logging.warning(f"Ошибка загрузки шрифта: {str(e)}, используется дефолтный")
...
def text_to_speech(text: str, lang: str = 'ru') -> tuple[bytes, float, str]:
    try:
        ...
        return audio_data, duration, temp_audio_path
    except Exception as e:
...
def create_animation(text: str, duration: float, audio_path: str) -> bytes:
    ...
    video_size_buffer = len(video_bytes.getvalue()) / (1024 * 1024)
    logging.info(f"Размер видео (в буфере): {video_size_buffer:.2f} МБ")
...
async def on_startup() -> None:
    logging.info("Начало выполнения on_startup")
    webhook_url = f"https://{RENDER_EXTERNAL_HOSTNAME}/webhook"
